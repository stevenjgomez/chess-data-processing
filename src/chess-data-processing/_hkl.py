...
try:
    _libhkl = np.ctypeslib.load_library('libhkl', '..')
except OSError:
    raise FileNotFoundError(f"libhkl.dll library not found in {folder_path}. See README for instructions on compiling "
                            f"c code.")
...
# void hist(double *Hbin,double *Kbin,double *Lbin,double *HR,double *KR,double *LR,double *counts,double *vol,
# double *norm,int N,int hn,int kn,int ln);
...
def HISTARB(IN, counts, mon, Q1bin, Q2bin, phat, vec1, vec2, prangemin, prangemax, vol, norm):
    ...
    _libhkl.histarb(Q1bin, Q2bin, phat, HR, KR, LR, vec1, vec2, prangemin, prangemax, I, vol, norm, N, lenQ1, lenQ2,
                    mon)
...
def CalcB(a1, a2, a3):
    ...
    B = np.matrix([[B1, 0, 0], [B2 * np.cos(beta3), B2 * np.sin(beta3), 0.0],
                   [B3 * np.cos(beta2), -B3 * np.sin(beta2) * np.cos(alpha1), 1.0 / A3]])
    return B
